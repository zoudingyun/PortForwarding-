...
namespace PortControllerClient.Forms
{
    public partial class changeName : Form
    {
        ...
        private void send_Click(object sender, EventArgs e)
        {
            ...
            send.Add("TYPE","CHANGENAME");
            send.Add("USER",PublicVariable4CS.UserName);
            send.Add("PWD",this.pwd.Text);
            send.Add("NEWDESC",this.name.Text);
            ...
            {
                if ((string)messageTable["TYPE"] == "CHANGENAME-RE")
                {
                    if (messageTable.Contains("RE_ANSWER"))
                    {
                        ...
                        {
                            MessageBox.Show("昵称修改失败：" + (string)messageTable["RE_MESSAGE"]);
                        }
                    }
                    else
                    {
